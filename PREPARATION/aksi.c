        ...
        totalAksi di-inkremen, totalDuration, dan totalCost ditambahkan sesuai dengan durasi dan biaya aksi A */
{
	...
	if (AksiNomor(A) != 3){
		for (i = 0; i < 3; i++){
			totalBahan[i] += RideBCost(AksiTarget(A))[i];
		}
	}
}
...
/* Mengembalikan true jika bahan masih tersisa setelah melakukan aksi A */
{
	...
	/* Algoritma */
	for (i = 0; i < 3; i++){
		if ((totalBahan[i] + RideBCost(AksiTarget(A))[i] - CurrentBahan[i]) > 0){
			return false;
		}
	}
	return true;
...
/* Mengembalikan true jika durasi masih tersisa setelah melakukan aksi A */
{
	/* Kamus */
	/* Algoritma */
	return (Durasi(tutup, buka) - totalDuration - AksiDuration(A) > 0);
